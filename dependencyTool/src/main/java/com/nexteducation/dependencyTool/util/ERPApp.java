...
final public class ERPApp {
	...
	@SuppressWarnings("unchecked")
	public static <S, T> List<T> PropertyMethodToValueTransform(final List<S> source, final String value) {
		if (ERPApp.isEmptyString(value) || ERPApp.isNullOrEmptyList(source)) {
			return null;
		}
		List<T> list = (List<T>) CollectionUtils.collect(source,
				TransformerUtils.invokerTransformer("get" + value.substring(0, 1).toUpperCase() + value.substring(1)));
		return list;
	}

	public static <S> List<String> listToString(final List<S> oldList) {
		if (ERPApp.isNullOrEmptyList(oldList)) {
			return null;
		}
